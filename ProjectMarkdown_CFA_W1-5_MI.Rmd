...
for (i in seq_along(WaveAdict_singleitem)){
  cat(sprintf("Variable: %s:\t\t\tDescription: %s\n", names(WaveAdict_singleitem)[i], WaveAdict_singleitem[[i]][1]))
  }
for (i in seq_along(WaveAdict_multiitem)){
  cat(sprintf("Variable: %s:\t\t\tDescription: %s\n", names(WaveAdict_multiitem)[i], WaveAdict_multiitem[[i]][1]))
}
...
M1 <- matchit(as.formula(forM), data=matchData, method= "nearest", discard="both",
              caliper = .20, ratio=3, distance="logit", distance.options=list(maxit=100), exact="xcsl2003")
