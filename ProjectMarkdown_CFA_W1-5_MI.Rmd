...
FunPlot_Multi <- function(model=fitQG, data=lifeSat, main="Life Satisfaction", paras){
  #Observed Means
...
#drop location
forM <- paste("group ~ ", paste0(names(matchData)[-c(1,18, 37)],collapse=" + ") )
M1 <- matchit(as.formula(forM), data=matchData, method= "nearest", discard="both",
              caliper = .15, ratio=3, distance="logit", distance.options=list(maxit=100),
              exact="schoolid")
SM1 <- summary(M1,  addlvariables=NULL, standardize=TRUE)#interactions=TRUE,
...
-------------------------------
Give the scale and complexity of the missing data model, it was run seperately on a super computer and is loaded here.
```{r MI, echo=FALSE, cache=TRUE}
#Bind with analysis Data

data<- merge(data, matchData, by="id", all.x=TRUE)
data <- data[, -c(grep("^W[6-7]|^W.[1-9]|.y$|xath2003|^pv[2-5]", names(data)))]
data[,c("escs", "xhrs2003", "xwkp2003", "instmot", "matheff", "belong", "intmat",
               "anxmat", "scmat", "complrn", "cooplrn", "teachsup", "disclim")]<- 
  apply(data[,c("escs","xhrs2003", "xwkp2003", "instmot", "matheff", "belong", "intmat",
               "anxmat", "scmat", "complrn", "cooplrn", "teachsup", "disclim")], 2, 
      function(x) recode(x,"1000=NA; 999=NA;998=NA")
      )
round(apply(data,2, function(x) sum(is.na(x))/nrow(data)),3)
```
